...
SELECT
	...
	r.relname AS index_name,
	ROUND(MAX(CASE WHEN s.fac = 1 THEN r.relsize END) / 1024.0 / 1024.0, 2) AS "ovw_size_mb",
	ROUND(SUM(r.relsize * s.fac) / 1024.0,2) ||' ('||ROUND(SUM(r.relsize * s.fac) / (COALESCE(NULLIF(MAX(CASE WHEN s.fac = -1 THEN r.relsize END),0.0),8192.0) / 100.0)::numeric,2)||'%)' AS "ovw_lag_size_kb"
FROM s
	INNER JOIN sn_relations r ON r.snap_id=s.snap_id
