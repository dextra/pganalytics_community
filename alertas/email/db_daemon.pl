...
while ($keep_going) 
{
	...
	{	 
		print STDERR "[EMAIL DATA -----------------------------------------------------------------------------------------------------------]\n" if $DEBUG;
		print STDERR "[db_daemon] Enviando email para os alertas: ". join(':',split('\n',$sarr_alert_id)) ."\n" if $DEBUG;
...
sub send_email
{
	...
	my $email_to = $se_args[0];
	my $email_subject = $se_args[1];
	my $email_body = $se_args[2];
...
#	$att->[1]->{file}='./Attachment_TESTE.txt';
	...
	my $retmail = $mail->send(-to=>$email_to,
				-bcc=>'email@mydomain.com',
                               -subject=>$email_subject,
                               -body=>$email_body,
            	               -contenttype=>'text/html'   );
#                    -attachmentlist=>$att);
      	print STDERR "[db_daemon] ===== Retorno do email <$retmail>\n" if $DEBUG;  
	return $retmail;
